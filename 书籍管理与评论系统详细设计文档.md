...
│   │   │   │   ├── Category.java                   # 分类实体
│   │   │   │   ├── Comment.java                    # 评论实体
│   │   │   │   ├── UserFavoriteBook.java           # 用户收藏书籍实体
...
│   │   │   │   ├── UserFavoriteBookRepository.java # 用户收藏书籍数据访问
...
│   │   │   │   ├── UserFavoriteBookService.java    # 用户收藏书籍服务接口
│   │   │   │   └── impl/
│   │   │   │       ├── UserServiceImpl.java        # 用户服务实现
│   │   │   │       ├── BookServiceImpl.java        # 书籍服务实现
│   │   │   │       ├── CommentServiceImpl.java     # 评论服务实现
│   │   │       │   ├── UserFavoriteBookServiceImpl.java # 用户收藏书籍服务实现
...
public class User {
    ...
    private List<UserFavoriteBook> favorites = new ArrayList<>();
...
public interface UserFavoriteBookService {
     ...
     * @return 收藏的书籍列表
     */
    List<Book> getUserFavorites(Long userId);
...
public class BookController {
    ...
    public String uncollectBook(@PathVariable Long id) {
        // 取消收藏书籍
    }
...
public class CommentLikeController {
    ...
    @PostMapping("/like/{id}")
    public ResponseEntity<?> likeComment(@PathVariable("id") Long id, @AuthenticationPrincipal User user) {
        commentService.likeComment(id, user.getId());
        return ResponseEntity.ok(Map.of("success", true, "message", "点赞成功"));
    }
    
    @PostMapping("/unlike/{id}")
    public ResponseEntity<?> unlikeComment(@PathVariable("id") Long id, @AuthenticationPrincipal User user) {
    ...
    public ResponseEntity<?> getLikeStatus(@PathVariable("id") Long id, @AuthenticationPrincipal User user) {
        boolean isLiked = commentService.isLikedByUser(id, user.getId());
        int likeCount = commentService.getLikeCount(id);
        return ResponseEntity.ok(Map.of("liked", isLiked, "count", likeCount));
    }
}
...
CREATE TABLE `user_favorite_books` (
  ...
  CONSTRAINT `fk_user_favorite_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_user_favorite_book_id` FOREIGN KEY (`book_id`) REFERENCES `books` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```
...
CREATE TABLE `comment_likes` (
  ...
  CONSTRAINT `fk_comment_like_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_comment_like_comment_id` FOREIGN KEY (`comment_id`) REFERENCES `comments` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```
...
| 取消收藏 | POST | /books/uncollect/{id} | id | 成功消息 | 取消收藏书籍 |
| 我的收藏 | GET | /user/favorites | 无 | favorites.html | 查看我的收藏 |
...
| 评论点赞 | POST | /comments/like/{id} | id | 成功消息 | 点赞评论 |
| 取消点赞 | POST | /comments/unlike/{id} | id | 成功消息 | 取消点赞评论 |
| 获取点赞状态 | GET | /comments/like/status/{id} | id | JSON | 获取评论点赞状态 |
