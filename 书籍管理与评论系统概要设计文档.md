...
[Thymeleaf模板] --> [UserFavoriteBooksController]

[UserController] --> [UserService]
[BookController] --> [BookService]
[CommentController] --> [CommentService]
[UserFavoriteBooksController] --> [UserFavoriteBooksService]

[UserService] --> [UserRepository]
[BookService] --> [BookRepository]
[CommentService] --> [CommentRepository]
[UserFavoriteBooksService] --> [UserFavoriteBooksRepository]
[CommentService] --> [CommentLikesRepository]

[UserRepository] --> (MySQL数据库)
[BookRepository] --> (MySQL数据库)
[CommentRepository] --> (MySQL数据库)
[UserFavoriteBooksRepository] --> (MySQL数据库)
[CommentLikesRepository] --> (MySQL数据库)
@enduml
```
...
- `UserFavoriteBooks`：用户收藏实体类
- `UserFavoriteBooksService`：收藏业务逻辑接口
- `UserFavoriteBooksRepository`：收藏数据访问接口

#### 3.2.3 主要流程
- 书籍添加流程：接收书籍信息 → 验证信息 → 保存书籍信息
- 书籍查询流程：接收查询条件 → 构建查询 → 执行查询 → 返回结果
- 书籍收藏流程：接收收藏请求 → 验证权限 → 创建用户收藏记录
...
- `CommentLikesRepository`：评论点赞数据访问接口

#### 3.3.3 主要流程
- 评论发布流程：接收评论内容 → 验证权限 → 保存评论信息
- 评论点赞流程：接收点赞请求 → 验证权限 → 创建/取消点赞记录
...
#### 4.2.5 user_favorite_books表

| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
|-------|---------|------|------|------|
| id | BIGINT | 20 | 主键，自增 | 收藏记录ID |
| user_id | BIGINT | 20 | 外键，非空 | 用户ID |
| book_id | BIGINT | 20 | 外键，非空 | 书籍ID |
| created_at | DATETIME | | 非空 | 收藏时间 |

#### 4.2.6 comment_likes表

| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
|-------|---------|------|------|------|
| id | BIGINT | 20 | 主键，自增 | 点赞记录ID |
| user_id | BIGINT | 20 | 外键，非空 | 用户ID |
| comment_id | BIGINT | 20 | 外键，非空 | 评论ID |
...
entity "user_favorite_books" as user_favorite_books {
  + id : BIGINT
  --
  + user_id : BIGINT
  + book_id : BIGINT
...
users ||--o{ comments : "发表"
users ||--o{ user_favorite_books : "收藏"
users ||--o{ comment_likes : "点赞"
users ||--o{ user_roles : "拥有"
books ||--o{ comments : "被评论"
books ||--o{ user_favorite_books : "被收藏"
...
- `/books/search`：搜索书籍
- `/books/favorite/{id}`：收藏书籍
- `/books/favorites`：我的收藏（需要管理员权限）
...
#### 6.1.4 UserFavoriteBooksController
- `/user/favorites`：查看我的收藏
- `/user/favorites/delete/{id}`：取消收藏

#### 6.1.5 AdminController
