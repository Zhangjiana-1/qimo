...
### 4.5 user_favorite_books表（用户收藏书籍表）
存储用户收藏的书籍信息。
...
CREATE TABLE `user_favorite_books` (
  ...
  KEY `fk_user_favorite_books_book_id` (`book_id`),
...
### 4.6 comment_likes表（评论点赞表）
存储用户对评论的点赞信息。

| 字段名 | 数据类型 | 长度 | 约束 | 描述 |
|-------|---------|------|------|------|
| id | BIGINT | 20 | 主键，自增 | 点赞ID |
| user_id | BIGINT | 20 | 外键，非空 | 用户ID |
...
CREATE TABLE `comment_likes` (
  ...
  KEY `fk_comment_likes_comment_id` (`comment_id`),
...
- user_favorite_books表：user_id和book_id组合设置唯一索引，确保一个用户只能收藏一本书一次
- comment_likes表：user_id和comment_id组合设置唯一索引，确保一个用户只能对一条评论点赞一次
...
- user_favorite_books表：book_id字段设置普通索引，优化按书籍查询收藏记录效率
- comment_likes表：comment_id字段设置普通索引，优化按评论查询点赞记录效率
...
LEFT JOIN user_favorite_books fav ON b.id = fav.book_id
...
### 7.4 sp_toggle_comment_like存储过程
用于切换评论点赞状态（点赞/取消点赞）的存储过程。
...
BEGIN
  -- 检查是否已点赞
  IF EXISTS (SELECT 1 FROM comment_likes WHERE user_id = userId AND comment_id = commentId) THEN
    -- 取消点赞
    DELETE FROM comment_likes WHERE user_id = userId AND comment_id = commentId;
    SELECT 0 AS liked_status;
  ELSE
    -- 添加点赞
    INSERT INTO comment_likes (user_id, comment_id, created_at)
    VALUES (userId, commentId, NOW());
    SELECT 1 AS liked_status;
  END IF;
END $$
DELIMITER ;

### 7.5 sp_get_comment_like_status存储过程
获取用户对特定评论的点赞状态的存储过程。

```sql
DELIMITER $$
CREATE PROCEDURE sp_get_comment_like_status(
  IN userId BIGINT,
  IN commentId BIGINT
)
BEGIN
